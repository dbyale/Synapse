...
export function createChatFunctions(defineFn: DefineFn) {
  return {
    ...
    python_environment_info: defineFn({
      description:
        'Check whether Python is installed and available on this system. ' +
        'Returns the Python binary name and version string. ' +
        'Call this before run_python if you are unsure whether Python is available. ' +
        'NOTE: run_python is intended for short computational snippets only — ' +
        'calculations, sorting, data transformations, and algorithm verification. ' +
        'Do not call this tool if you only intend to write a full program for the user to save; ' +
        'use write_file for that instead.',
      params: {
        type: 'object',
        properties: {},
      },
    ...
    run_python: defineFn({
      ...
      async handler(params: { code: string }) {
        ...
        // Return a clean, structured response the model can reason about
        if (!result.success) {
          return {
            success: false,
