...
export class MemoryManager {
  ...
  async readMemory(title: string): Promise<{
    success: boolean;
    title?: string;
    type?: string;
    tags?: string[];
    ...
    try {
      ...
      if (!existsSync(filePath))
        return {
          success: false,
          error: `Memory "${title.trim()}" not found. Use list_memories or search_memories to find available memories.`,
        };
      ...
      const {
        ...
        modified,
        content,
      } = await this.readFileMeta(filePath);
      return {
        success: true,
        title: memTitle,
        type,
        tags,
        content,
        related_memories: extractWikiLinks(content),
        created,
        modified,
      };
    } catch (error) {
      ...
      for (const file of files) {
        ...
        try {
          const {
            ...
            modified,
            content,
          } = await this.readFileMeta(join(this.baseDir, file));
          const q = (query || '').toLowerCase();
          const matchesQuery =
            !query ||
            title.toLowerCase().includes(q) ||
            fileType.toLowerCase().includes(q) ||
    ...
    try {
      ...
      if (!existsSync(filePath))
        return {
          ...
          title: title.trim(),
          deleted: false,
          error: `Memory "${title.trim()}" not found.`,
        };
      ...
      for (const file of files) {
        ...
        try {
          ...
          if (matchesType && matchesTag)
            memories.push({ title, type: fileType, tags, created, modified });
        } catch {
          // skip
        }
      }
