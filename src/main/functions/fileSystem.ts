  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) {
      nodes.push({
        name: entry.name,
        type: 'directory',
        children: await buildDirectoryTree(fullPath, excludePatterns),
      });
    } else {
...
export async function readTextFile(params: {
  ...
  try {
    ...
    if (params.head !== undefined && params.head > 0)
...
export async function listDirectoryWithSizes(params: {
  path: string;
  sortBy?: string;
}): Promise<string> {
  try {
    ...
    return (
      items
        .map(
          (i) =>
            `${i.type.padEnd(4)} ${i.size.toString().padStart(10)} ${i.name}`,
        )
        .join('\n') || '(empty directory)'
    );
  } catch (error) {
