...
export function parseQuantization(
  ...
  if (isProjector || upperQ === 'MMPROJ') {
    info.details.push('Multimodal Projector (used for vision/image capabilities)');
  }
  ...
  if (baseMatch) {
    ...
    else if (prefix === 'IQ') info.details.push(`${bitText} I-Quant (Importance Matrix, higher quality)`.trim());
    else if (prefix === 'FP' || prefix === 'F') info.details.push(`${bitText} Floating Point (Unquantized precision)`.trim());
    else if (prefix === 'BF') info.details.push(`${bitText} Brain Floating Point (AI-optimized precision)`.trim());
  ...
  for (const token of tokens) {
    switch (token) {
      ...
      // Sizes
      case 'XXS': info.details.push('Extra Extra Small (maximum compression)'); break;
      case 'XS': info.details.push('Extra Small'); break;
      case 'S': info.details.push('Small (favors smaller file size)'); break;
      case 'M': info.details.push('Medium (recommended balance)'); break;
      case 'L': info.details.push('Large (favors quality over size)'); break;
      case 'XL': info.details.push('Extra Large (maximum quality for this bit-rate)'); break;
    }
  }
